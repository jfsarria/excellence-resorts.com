...
if (count($errors)==0) {
  ...
  for ($ROOM_NUM=1;$ROOM_NUM<=(int)$num_rooms; ++$ROOM_NUM) {
    $QRYSTR['RES_ROOM_'.$ROOM_NUM.'_ADULTS_QTY'] = $num_adults;
  ...
  foreach ($hotels as $h => $hotel) {
    ...
    if ($partner_id=="1") {
      if ($_SERVER["HTTP_HOST"]=="excellence-resorts.com" || strstr($_SERVER["HTTP_HOST"],"205.186.160.44")!==FALSE) {
      ...
      $ROOMS_URL = "http://www.excellence-resorts.com/guest-suites/suites-at-punta-cana";
        ...
        $QRYSTR['RES_PROP_ID'] = "6";
        $ROOMS_URL = "http://www.excellenceresorts.com/resorts/excellence-el-carmen/suites/";
    }
    ...
    if ($QRYSTR['RES_PROP_ID'] == "0") {
      ...
      $ibe_url = "http://" . $_SERVER["HTTP_HOST"] . "/ibe/index.php?" . http_build_query($QRYSTR);
      /*
      if ($_SERVER["HTTP_HOST"]=="excellence-resorts.com") {
        $json = file_get_contents('http://excellence-resorts.com/ibe/index.php?' . http_build_query($QRYSTR));
      } else {
        $json = file_get_contents('http://secure-belovedhotels.com/ibe/index.php?' . http_build_query($QRYSTR));
      }
      */

      //print $ibe_url;exit;
      $json = file_get_contents($ibe_url);
      //mail("jaunsarria@gmail.com","EX ibe_url",$ibe_url);
