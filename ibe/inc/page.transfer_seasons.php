...
if ($ACTION=="EDIT") {
    $RSET = $clsTransferSeasons->getById($db, array("SEASON_ID"=>$SEASON_ID));
    if ( $RSET['iCount'] != 0 ) {
...
if ($ACTION=="SAVE") {
    ...
    if (isset($error) && sizeof($error) != 0) {
        ...
        //print "<pre>";print_r($_DATA);print "</pre>";
        $result = $clsTransferSeasons->save($db, $_DATA); 

        if ((int)$result == 1) {
            include_once "inc/ibe.frm.ok.php";
            print "
                <script>
                    document.location.href = '?PAGE_CODE=transfer_seasons&PROP_ID={$PROP_ID}';
                </script>
            ";
        } else {
...
if ($ACTION=="LIST") { 
    include_once "page.transfer_seasons.list.php";
} else { 
    ?>
    <form id="editfrm" method="post" enctype="multipart/form-data">
        ...
        <div class='frmBtns' style="padding-bottom:0;">
            ...
            <a href="?PAGE_CODE=transfer_seasons&PROP_ID=<? print (isset($PROP_ID)) ? $PROP_ID : "1" ?>"><span class="button plain">Go Back</span></a>
        </div>
