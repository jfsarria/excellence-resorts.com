...
$DATA_START = "1306886400"; //Wed, 01 Jun 2011 00:00:00 GMT
?>
...
<tbody><tr>
        ...
        <fieldset>
            ...
            <div class="fieldset">
                <div class="label">
					...
					<div><span <?if(in_array('4',$_GET['PROP_IDs'])){?>class="selectedItem"<?}?>><input type="checkbox" <?if(in_array('4',$_GET['PROP_IDs'])){?>checked=""<?}?> id="PROP_IDs_4" value="4" name="PROP_IDs[]"></span>&nbsp;The Beloved Hotel</div>     
				</div>
            </div>
        </fieldset>
    ...
    <td width="80%" valign="top">
		...
		<script type="text/javascript" src="/lmd-test/js/jquery-ui.min.js"></script>	
		<link  rel="stylesheet" type="text/css" href="/lmd-test/style_calendar-ibe.css" />
		<script>
...
if($_GET['SRCH']) {
	...
	/* ***************
	   ...
	   *************** */	
		...
		for($YEAR_NOW = $YEAR_S; $YEAR_NOW<=$YEAR_F; $YEAR_NOW++) {
			foreach($HOTEL as $tbl_name) {
				$sql = "SELECT `restbl`.`ID`, `restbl`.`NUMBER`, `restbl`.`CHECK_IN`, `restbl`.`CHECK_OUT`, `restbl`.`NIGHTS`, `restbl`.`ROOMS`, `restbl`.`ADULTS`, `restbl`.`CHILDREN`, `restbl`.`TOTAL`, `restbl`.`METHOD`, `restbl`.`CANCELLED`, `restbl`.`CREATED`, `restbl`.`STATUS` , `restbl`.`SOURCE_ID`, '".$tbl_name."', `restbl`.`GEO_COUNTRY_CODE`, `restbl`.`CLASS_NAMES`, `restbl`.`SPECIAL_NAMES`, 	
