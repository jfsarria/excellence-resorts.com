...
if ($DELETE_ID!=0) {
    $result = $clsTransferSeasons->remove($db, array("DELETE_ID"=>$DELETE_ID));
    if ((int)$result==-1) {
...
?>
<form id="lstFrm">
<input type="hidden" name="PAGE_CODE" value="transfer_seasons">
...
<div class='ListBtns'>
    <table width="800">
    <tr>
        <td><h2>Transfer Seasons</h2></td>
        <td><a href="?PAGE_CODE=transfer_seasons&PROP_ID=<? print $PROP_ID ?>&SEASON_ID=0"><span class="button key">Add New</span></a></td>
        <td>Year:
...
<?
$RSET = $clsTransferSeasons->getByProperty($db, array("PROP_ID"=>$PROP_ID,"YEAR"=>$_YEAR));
if ( $RSET['iCount'] != 0 ) { ?>
    <div class="listTbl">
    <table>
    <tr class="listRowHdr">
    ...
    while ($row = $db->fetch_array($RSET['rSet'])) {
        ...
        $EDIT = "?PAGE_CODE=transfer_seasons&PROP_ID={$row['PROP_ID']}&SEASON_ID={$row['ID']}";
        ?>
