...
if ($DELETE_ID!=0) {
    $result = $clsFlashsale->archive($db, array("DELETE_ID"=>$DELETE_ID),true);
    if ((int)$result==-1) {
...
?>
<form id="lstFrm">
<input type="hidden" name="PAGE_CODE" value="flashsale">
...
<div class='ListBtns'>
    <table width="940">
    <tr>
        <td><h2>Flash Sale</h2></td>
        <td><a href="?PAGE_CODE=flashsale&PROP_ID=<? print $PROP_ID ?>&ID_CAB=0"><span class="button key">Add New</span></a></td>
...
<?
$RSET = $clsFlashsale->getByProperty($db, array("PROP_ID"=>$PROP_ID,"WEHRE"=>"AND SYSTEM LIKE 'F_%' AND IS_ARCHIVE='{$_IS_ARCHIVE}'"));
if ( $RSET['iCount'] != 0 ) { ?>
    <div class="listTbl">
    <table>
    <tr class="listRowHdr">
    ...
    while ($row = $db->fetch_array($RSET['rSet'])) {
        ...
        <tr class='listRow<? print $cnt ?>'>
            <td><a href="?PAGE_CODE=flashsale&PROP_ID=<? print $row['PROP_ID'] ?>&ID_CAB=<? print $row['ID_CAB'] ?>"><? print $NAME ?></a></td>
            ...
            <td align='right'><? 
             $fecha= new DateTime($row['WIN_FROM']);
             $form_fecha= $fecha->format('d-m-Y');  
            print $form_fecha; ?></td>

            <td align='right'><?
             $fecha= new DateTime($row['WIN_TO']);
             $form_fecha= $fecha->format('d-m-Y');  
            print $form_fecha;  ?></td>
