...
if (!isset($_SESSION['AVAILABILITY']['RESERVATION']['DONE']['PAYMENT']) && $_SESSION['AVAILABILITY']['RESERVATION']['RES_GUESTMETHOD']=="CC") {
    if (isset($_SESSION['AVAILABILITY']['RESERVATION']['RES_NUMBER']) && trim($_SESSION['AVAILABILITY']['RESERVATION']['RES_NUMBER'])!="") {
        ...
        if ((int)$RES_PROP_ID==4) $_PUBLISHER_NAME = "excellence4"; // Beloved
        if ((int)$RES_PROP_ID==5) $_PUBLISHER_NAME = "excellence1"; // FPM
        if ((int)$RES_PROP_ID==6) $_PUBLISHER_NAME = "excellence5"; // XEC
        if ((int)$RES_PROP_ID==7) $_PUBLISHER_NAME = "excellence6"; // XOB

        $CHILDREN = (isset($_SESSION['AVAILABILITY']['RES_ROOMS_CHILDREN_QTY']) && (int)$_SESSION['AVAILABILITY']['RES_ROOMS_CHILDREN_QTY']!=0) ? (int)$_SESSION['AVAILABILITY']['RES_ROOMS_CHILDREN_QTY'] : 0;
        $INFANTS = (isset($_SESSION['AVAILABILITY']['RES_ROOMS_INFANTS_QTY']) && (int)$_SESSION['AVAILABILITY']['RES_ROOMS_INFANTS_QTY']!=0) ? (int)$_SESSION['AVAILABILITY']['RES_ROOMS_INFANTS_QTY'] : 0;
        ...
        $CCDATA = array(
            ...
            "card_amount"=>(int)$RESERVATION["RES_TOTAL_CHARGE"] + (int)$RESERVATION["TRANSFER_FEE"],
