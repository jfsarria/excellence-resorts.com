...
if ($SUBMIT=="SUBMIT") {
    ...
    if ($MODIFY=="CANCEL" || $MODIFY=="NOSHOW" || $MODIFY=="SUPPLEMENT" || $MODIFY=="TRANSFER" || $MODIFY=="TRANSFER_CANCEL") {
        ...
        $ALREADY_CHARGED = isset($_DATA['ALREADY_CHARGED']) ? (int)$_DATA['ALREADY_CHARGED'] : 0;
        ...
        if ($MODIFY=="TRANSFER" || $MODIFY=="TRANSFER_CANCEL") {
            ...
            if (isset($_DATA['RES_GUEST_TRANSFER_FEE'])) {
              $TRANSFER['TRANSFER_FEE'] = $_DATA['RES_GUEST_TRANSFER_FEE'];
              $TRANSFER_FEE = (int)$TRANSFER['TRANSFER_FEE'];
            }
            if (isset($_DATA['CC_COMMENTS'])) $TRANSFER['CC_COMMENTS'] = $_DATA['CC_COMMENTS'];
        ...
        if ((int)$result == 1) {
            ...
            ob_start();
                ...
                if ($MODIFY=="SUPPLEMENT" || $TRANSFER_FEE!=0 || $ALREADY_CHARGED==0) {
                    $CCDATA = array (
                        "ONLY_PENDING"=>1,
