    ...
    <fieldset>
        ...
        <div class="fieldset">
            ...
            <? foreach ($RESERVATION['RES_ROOMS_SELECTED'] as $ind => $ROOM_ID) { 
                ...
                <? if ($RES_ROOMS_QTY>1) { ?>
                    <div>No. of Guests: <? print $GUESTS ?></div>
                <? } ?>
                <div>Daily Charges</div>
                <? //print "<!--";print_r($ROOM['NIGTHS']);print "-->"; ?>
                <? foreach ($ROOM['NIGTHS'] as $NIGHT => $NIGHT_DATA ) { 
                    //print "<!-- NIGHT_DATA:\n";print_r($NIGHT_DATA);print "-->";
                    if (is_array($NIGHT_DATA)) {
                        $GROSS = (int)$NIGHT_DATA['RATE']['GROSS']*$GUESTS;
                        $FINAL = (int)$NIGHT_DATA['RATE']['FINAL']*$GUESTS;
                        ?>
            ...
            <? } ?>
            <br>
            <div><b>Total Room Charge: USD <? print number_format($RESERVATION['RES_TOTAL_CHARGE']) ?></b></div>
            <? if (isset($RESERVATION['SUPPLEMENT']) && trim($RESERVATION['SUPPLEMENT'])!="") {  ?>
                ...
                <div><b>Final Room Charge USD <? print number_format((int)$RESERVATION['RES_TOTAL_CHARGE'] + (int)$RESERVATION['SUPPLEMENT']) ?></b></div>
            <? } else {
              $RESERVATION['SUPPLEMENT'] = 0;
            } ?>
            ...
            <? } else { 
              ...
              if ($CAN_ADD_TRANSFER) { ?>
                  ...
                  <div style='text-align:center'>
                    <a href="<? print $THIS_PAGE."&MODIFY=TRANSFER"; ?>"><span class="button key">Add Private Airport Transfer (1)</span></a>
                    <br><br>
                  </div>
              <? } ?>
            <? } ?>
            <br>
            <div style='font-size:16px'><b>Total Charge: USD <? print number_format((int)$RESERVATION['RES_TOTAL_CHARGE'] + (int)$RESERVATION['SUPPLEMENT'] + (int)$RESERVATION['TRANSFER_FEE']) ?></b></div>
        </div>
    </fieldset>
<? } ?>