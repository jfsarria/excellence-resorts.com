    ...
    //print "6. $sql<br><br>";
		...
		for($YEAR_NOW = $YEAR_S; $YEAR_NOW<=$YEAR_F; $YEAR_NOW++) {
			foreach($HOTEL as $tbl_name) {
				...
				`GUESTS`.`FIRSTNAME`, `GUESTS`.`LASTNAME`, `GUESTS`.`EMAIL`, `GUESTS`.`CITY`, `GUESTS`.`STATE`, `GUESTS`.`COUNTRY`, `GUESTS`.`TITLE`, `restbl`.`ARRAY`
						
						FROM `RESERVATIONS_".$tbl_name."_".$YEAR_NOW."` as `restbl`, `GUESTS`						
						
						WHERE `GUESTS`.`ID` = `restbl`.`GUEST_ID` ".$W;
                print_sql($YEAR_NOW."<br>".$sql, "5");
        //print "7. $sql<br><br>";
				$r = mysql_query($sql);// or die(mysql_error().' SQL: '.$sql);
				$datat = array();
				for($datat=array();$row=@mysql_fetch_assoc($r);$datat[]=$row);
        ...
        $MAX_ROOMS = 0;    
        foreach($data as $el) {
           $MAX_ROOMS = $el['ROOMS'] > $MAX_ROOMS ? $el['ROOMS'] : $MAX_ROOMS;
        }
		?>
		<table>
			<tr>
                ...
                <th>ROOM <?=$r?> INFANT</th>
                <th>ROOM <?=$r?> CHILD 1</th>
                <th>ROOM <?=$r?> CHILD 2</th>
                <th>ROOM <?=$r?> CHILD 3</th>
                <th>ROOM <?=$r?> CHILD 4</th>
                <th>ROOM <?=$r?> CHILD 5</th>
                <? } ?>
			</tr>
			<?foreach($data as $el) {?>
				<tr>
					...
					<th><?
						...
						if($el['XEC']) $tmp = $el['XEC'];
            if($el['XOB']) $tmp = $el['XOB'];
						...
						if($tmp=='XEC') {?>Excellence El Carmen<?}
            if($tmp=='XOB') {?>Excellence Oyster Bay<?}
						?></td>
					<td><?$tmp=array();
                    ...
                    <?
                    if ($el['CHILDREN']!=0) {
                        $ARRAY = $clsGlobal->jsonDecode($el['ARRAY']);
                        if (is_array($ARRAY)) {
                            for ($ROOM_NUM=1; $ROOM_NUM <= $el['ROOMS']; ++$ROOM_NUM) {
                                $AGES = ARRAY(0);
                                $CHILDREN_QTY = (int)$ARRAY["RES_ROOM_{$ROOM_NUM}_CHILDREN_QTY"];
                                if ($CHILDREN_QTY!=0) {
                                    if ((int)$ARRAY["RES_ROOM_{$ROOM_NUM}_CHILD_AGE_5"]!=0) {
                                        $ARRAY["RES_ROOM_{$ROOM_NUM}_CHILD_AGE_{$CHILDREN_QTY}"] = 0;
                                        $ARRAY["RES_ROOM_{$ROOM_NUM}_CHILD_AGE_5"] = 0;
                                    }
                                    for ($CHILD_NUM=1; $CHILD_NUM <= $CHILDREN_QTY; ++$CHILD_NUM) {
                                        $AGE = (int)$ARRAY["RES_ROOM_{$ROOM_NUM}_CHILD_AGE_{$CHILD_NUM}"];
                                        if ($AGE > 3) {
                                            $AGES[] = $AGE;
                                        } else if ((int)$ARRAY["RES_ROOM_{$ROOM_NUM}_INFANTS_QTY"]!=0) {
                                            $AGES[0] = 1;
                                        }
                                    }
                                    $AGES = array_pad($AGES, 6, 0);
                                    //print "<td>".(implode(",",$AGES))."</td>";

                                    for ($CHILD_NUM=0; $CHILD_NUM < count($AGES); ++$CHILD_NUM) {
                                        $AGE = $AGES[$CHILD_NUM];
                                        print "<td>".($AGE==0?"":$AGE)."</td>";
                                    }
                                }
                            }
                        }
...
function print_sql($sql, $str) {
    //$out "<div style='padding:10px;background:#FFFFFF;color:#000000;border:solid 2px #000000'>{$str}<br>".$sql."</div>";
    //$out = "{$str} :: {$sql}";
    //mail("jaunsarria@gmail.com", "sql", $out);   
}