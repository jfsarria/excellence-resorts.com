...
if ($DELETE_ID!=0) {
    $result = $clsCoupon->remove($db, array("DELETE_ID"=>$DELETE_ID),true);
    if ((int)$result==-1) {
...
?>
<form id="lstFrm">
<input type="hidden" name="PAGE_CODE" value="coupon">
...
<div class='ListBtns'>
    ...
    <tr>
        <td><h2>Coupon</h2></td>
        <td><a href="?PAGE_CODE=coupon&PROP_ID=<? print $PROP_ID ?>&ID_CAB=0"><span class="button key">Add New</span></a></td>
...
<?
$RSET = $clsCoupon->getByProperty($db, array("PROP_ID"=>$PROP_ID,"WEHRE"=>"AND SYSTEM LIKE 'C_%' AND IS_ARCHIVE='{$_IS_ARCHIVE}'"));
if ( $RSET['iCount'] != 0 ) { ?>
    <div class="listTbl">
    <table>
    <tr class="listRowHdr">
    ...
    while ($row = $db->fetch_array($RSET['rSet'])) {
        ...
        <tr class='listRow<? print $cnt ?>'>
            <td>
                <a href="?PAGE_CODE=coupon&PROP_ID=<? print $row['PROP_ID'] ?>&ID_CAB=<? print $row['ID_CAB'] ?>"><? print $NAME ?></a>
            </td>
            ...
            <td>
                <img
                    src="css/img/remove_icon.gif"
                    width="10"
                    height="10"
                    border="0"
                    style="cursor:pointer"
                    onclick="$('#DELETE_ID').val(<? print $row['ID_CAB'] ?>);$('#lstFrm').submit()">
            </td>
