...
$ACTION = ($ACTION=="LIST"&&isset($_GET['CAR_ID'])) ? "EDIT" : $ACTION;

if ($ACTION=="EDIT") {
    $RSET = $clsTransfer->getCarById($db, array("CAR_ID"=>$CAR_ID));
    if ( $RSET['iCount'] != 0 ) {
...
if ($ACTION=="SAVE") {
    ...
    if (isset($error) && sizeof($error) != 0) {
        ...
        if (!isset($_DATA['CAR_ID']) || (int)$_DATA['CAR_ID'] == 0) {
            $CAR_ID = dbNextId($db);
            $_DATA['CAR_ID'] = $CAR_ID;
        }
        $_DATA['UPS_FOLDER'] = "/ibe/ups/transfers/";

        //print "<pre>";print_r($_DATA);print "</pre>";
        $result = $clsTransfer->saveCar($db, $_DATA); 

        if ((int)$result == 1) {
            /* Upload Images & Videos */
            foreach ($_FILES as $_KEY => $_FILE) $clsImage->upload($_KEY, $_DATA['UPS_FOLDER'], $CAR_ID);
...
if ($ACTION=="LIST") { 
    include_once "page.transfer_cars.list.php";
} else { ?>
    <form id="editfrm" method="post" enctype="multipart/form-data">
        ...
        <input type="hidden" name="CAR_ID" id="CAR_ID" VALUE="<? print (isset($CAR_ID)) ? $CAR_ID : "0" ?>">
        <div class='frmBtns' style="padding-bottom:0;">
            <a onclick="$('#editfrm').submit()"><span class="button key">Submit</span></a>
            <a href="?PAGE_CODE=transfer_cars&PROP_ID=<? print (isset($PROP_ID)) ? $PROP_ID : "1" ?>"><span class="button plain">Go Back</span></a>
        </div>
