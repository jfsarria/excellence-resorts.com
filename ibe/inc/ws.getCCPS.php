...
function getReservations($db, $arg) {
    ...
    foreach ($TABLES as $i=>$VIEWNAME) {
        ...
        $clsReserv->searchReservationQuery($db, array(
            ...
            "WHERE"=>"METHOD = 'CC' AND (".$WHERE.")"
        ), $qry);
    }

    $query = implode(" UNION ",$qry)." ORDER BY NUMBER DESC, ID DESC";
    $arg = array('query' => $query);
    //print "<p class='s_notice top_msg'><pre>$query</pre></p>";exit;
    $result = dbQuery($db, $arg);    return $result;
}
